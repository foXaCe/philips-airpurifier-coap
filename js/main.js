...
window.customIcons["pap"] = {
  getIcon: (iconName) => getIcon("pap", iconName),
  getIconList: () => getIconList("pap"),
};
...
customElements.whenDefined("ha-svg-icon").then(() => {
  ...
  HaSvgIcon.prototype.clearPaths = async function () {
    ...
    while (this.shadowRoot.querySelector("style.pap")) {
      const el = this.shadowRoot.querySelector("style.pap");
  ...
  HaSvgIcon.prototype.setPaths = async function (paths) {
    ...
    const styleEl =
      this.shadowRoot.querySelector("style.pap") ||
      document.createElement("style");
    styleEl.classList.add("pap");
    styleEl.innerHTML = `
      .secondary {
        opacity: 0.4;
      }
