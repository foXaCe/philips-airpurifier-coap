...
async def test_observe_stream_error_marks_unavailable(hass: HomeAssistant) -> None:
    ...
    The CoAP client is left untouched so control commands keep working; only the
    watchdog performs a full reconnect.
    """
    ...
    with patch(
        ...
        coord.async_add_listener(lambda: None)
        await hass.async_block_till_done()
    # A dead observe stream does NOT recreate the client (no churn); it just marks
    # the device unavailable until the watchdog reconnects.
